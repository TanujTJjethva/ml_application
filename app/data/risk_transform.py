...
def risk_transform_company_data() -> pd.DataFrame:
    ...
    df['volatility_pct'] = ((df['next_high'] - df['next_low']) / df['Close']) * 100

    # Stable target
    conditions = [
        (df['volatility_pct'] == 0),
        (df['volatility_pct'] > 0) & (df['volatility_pct'] <= 1),
        (df['volatility_pct'] > 1) & (df['volatility_pct'] <= 3),
        (df['volatility_pct'] > 3) & (df['volatility_pct'] <= 6),
        (df['volatility_pct'] > 6) & (df['volatility_pct'] <= 9),
        (df['volatility_pct'] > 9),
    ]
    ...
    df['target_volatility_risk'] = np.select(conditions, choices, default='unknown')
...
def risk_transform_broad_index_data() -> pd.DataFrame:
...
def risk_transform_sector_index_data() -> pd.DataFrame:
